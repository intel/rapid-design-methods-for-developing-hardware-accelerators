  ...
  private def deleteDeadCode(instMap: collection.Map[String, String],
    ...
    def onStmt(stmt: Statement): Statement = {
      val stmtx = stmt match {
        ...
        case con: Connect =>
//          println( s"Before getDeps: ${con.loc}")
          val node = getDeps(con.loc) match { case Seq(elt) => elt }
//          println( s"After getDeps: ${node}")
  ...
  def allRegs( modName : String)( s : Statement) : Statement = {
    ...
    s match {
      case DefRegister(_, name, tpe, clock, reset, init) =>
        ...
        println( s"Register: ${node} ${nodePS} ${nodeNS} ${tpe}")
  ...
  def run(state: CircuitState,
    ...
    println( s"Running DeadCodeElimination...")
    val c = state.circuit
...
/*
    ...
    println( s"Topo order")
    for { v <- depGraph.linearize} {
      println( s"\t${v}")
    }

    println( s"Topo order (reverse graph)")
    for { v <- reverseDepGraph.linearize} {
      println( s"\t${v}")
    }
 */
...
/*
    printCone( "SumOfSquares", "io_Q_valid", "io_Q_ready")
 */
    printCone( "LazyStackWait1", "io_Put_ready", "io_Put_valid")
    printCone( "LazyStackWait1", "io_Out_ready", "io_Out_valid")
    printCone( "LazyStackWait1", "io_Put_ready", "io_Put_bits")
    printCone( "LazyStackWait1", "io_Out_ready", "io_Out_bits")
    printCone( "LazyStackWait1", "io_In_valid",  "io_In_ready")
    printCone( "LazyStackWait1", "io_Get_valid", "io_Get_ready")
    printCone( "LazyStackWait1", "io_In_bits",  "io_In_ready")
    printCone( "LazyStackWait1", "io_Get_bits", "io_Get_ready")
    ...
    println( s"Finishing DeadCodeElimination...")

...
class HLS2VectorAdd2 extends ImperativeModule( 
  Compiler.runHLS2(
    """
      |process HLS2VectorAdd2 ( A : inp Vec(2,UInt(8)),
      |                        B : inp Vec(2,UInt(8)),
      |                        O : out Vec(2,UInt(8)))
      |{
      |  var a : Vec(2,UInt(8))
...
class HLS2VectorAdd2Tester[T <: ImperativeModule]( c : T) extends PeekPokeTester(c) {
  poke( c.io("O").ready, 1)
...
class HLS2VectorAdd2Test extends FlatSpec with Matchers {
  behavior of "HLS2VectorAdd2"
  it should "work" in {
    chisel3.iotesters.Driver( () => new HLS2VectorAdd2, "firrtl") { c =>
      new HLS2VectorAdd2Tester( c)
    } should be ( true)
  }
}

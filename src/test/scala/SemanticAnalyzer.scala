...
class SemanticAnalyzerTest extends FreeSpec with Matchers {
  "genLG" - {
    "empty" in {
      SemanticAnalyzer.genLG0( Blk( List(), List())) should be ( LGSeq( List()))
    }
    "while true" in {
      SemanticAnalyzer.genLG0( Blk( List(), List( While( ConstantTrue, Blk( List(), List()))))) should be ( LGSeq( List(LGWhileTrue(LGSeq(List())))))
    }
    "while not comm" in {
      SemanticAnalyzer.genLG0( Blk( List(), List( While( NotBExpression( NBCanGet( Port( "P"))), Wait)))) should be ( LGSeq( List(LGWhileNotProbeWait(NBCanGet(Port("P")),LGSeq(List()),(0,0)))))
    }
    "while not comm, then comm" in {
      SemanticAnalyzer.genLG0( Blk( List(), List( While( NotBExpression( NBCanGet( Port( "P"))), Wait), NBGet( Port( "P"), Variable( "x")), Wait))) should be ( LGSeq( List(LGWhileNotProbeWait(NBCanGet(Port("P")),LGSeq(List(LGPrim(NBGet(Port("P"),Variable("x"))),LGWait())),(0,0)))))
    }
    "assign and comm" in {
      ...
      SemanticAnalyzer.genLG0( blob) should be ( blob2)
    }
  }
  "assignLabels" - {
    "empty" in {
      SemanticAnalyzer.assignLabels( (0, LGSeq( List()))) should be ( 0, LGSeq( List()))
    }
    "while not comm, then comm" in {
      ...
      SemanticAnalyzer.assignLabels( (0, LGSeq( List(blob)))) should be (1, LGSeq( List(blob2)))
    }
    "double" in {
      val g = NBCanGet(Port("P"))
      val f = LGSeq(List(LGPrim(NBGet(Port("P"),Variable("x"))),LGWait()))
      ...
      SemanticAnalyzer.assignLabels( (0, LGSeq( List(blob,blob)))) should be (2, LGSeq( List(blob2,blob3)))
    }
    "assign and comm" in {
      val i = LGPrim(Assignment(Variable("x"),ConstantInteger(0)))
      val g = NBCanPut(Port("Q"))

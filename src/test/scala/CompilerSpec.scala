...
class CompilerSpec extends FlatSpec with Matchers {
  ...
  val successfulAST = Process(PortDeclList(List(PortDecl(Port("P"),Inp,UIntType(8)), PortDecl(Port("Q"),Out,UIntType(8)))),ResetWhileTrueWait(List(),List(),Blk(List(),List(IfThenElse(AndBExpression(NBCanGet(Port("P")),NBCanPut(Port("Q"))),Blk(List(Decl(Variable("v"),UIntType(8))),List(NBGet(Port("P"),Variable("v")), NBPut(Port("Q"),Variable("v")))),Blk(List(),List()))))))

...
class LazyStackNWait1Test extends FlatSpec with Matchers {
  ...
  it should "work" in {
    chisel3.iotesters.Driver.execute( Array( "-fct", "imperative.transform.ShannonFactor", "--backend-name", "verilator"), () => new LazyStackN(10, () => new LazyStackWait1)) { c =>
      new LazyStackNTester( c)
    } should be ( true)
  }
}
...
class LazyStackNWait2Test extends FlatSpec with Matchers {
  ...
  it should "work" in {
    chisel3.iotesters.Driver.execute( Array( "-fct", "imperative.transform.ShannonFactor", "--backend-name", "verilator"), () => new LazyStackN(10, () => new LazyStackWait2)) { c =>
      new LazyStackNTester( c)
    } should be ( true)
  }
}

...
//[[[end]]] (checksum: cdc606f2ac64f2c10ae7c6ff4937672a)
  ...
  unsigned int prev_gr = 0;

  bool validPipe0 = false;
  unsigned int prev_gr0 = 0;
  CacheLine m0mn[WAYS];
  CacheLine m0mx[WAYS];
  CacheLine m0o[WAYS];
  CacheLine m0[WAYS];
  ...
  unsigned int prev_gr1 = 0;
  CacheLine m1[WAYS];

  bool validPipe2 = false;
  unsigned int prev_gr2 = 0;
  CacheLine m2[WAYS>>2];

  bool validPipe3 = false;
  unsigned int prev_gr3 = 0;
  CacheLine m3[WAYS>>4];
  ...
  WHILE_ONE: while (1) {
    if ( start && !localDone) {
      ...
      bool cond3 = !( (( phase == 1) || ( phase == 2)) && validPipe3 && ( prev_gr3 == ngr-1)) || out.nb_can_put();
      ...
      */
      ...
      if ( cond0 && cond1 && cond2 && cond3) {
        if ( phase == 0) {
          ...
          if ( validPipe3) {
#if WAYS == 32
            best = vMin( vMin( m3[0], m3[1]), best, prev_gr3 == 0);
#else
            best = vMin( m3[0], best, prev_gr3 == 0);
#endif
            if ( prev_gr3 == ngr-1) {
              out.nb_put( best);
            }
          ...
          if ( validBuffer) { // !validPipe0
            for ( unsigned int j=0; j<elementsPerCL; ++j) {
              for ( unsigned int i=0; i<rowsPerClock; ++i) {
                short a = bufA[i].words[j];
                unsigned short e = bufE[i].words[j];
                unsigned int offOffset = (i+(prev_gr<<log2RowsPerClock))&(elementsPerCL-1);
                short o = offCL[(prev_gr<<log2RowsPerClock)>>log2ElementsPerCL].words[offOffset];
                m0mn[i].words[j] = ElementFunctionMn( a, prev_b);
                m0mx[i].words[j] = ElementFunctionMx( e, prev_f);
                m0o[i].words[j] = o;
              }
            }
            validBuffer = false;
          ...
          if ( phase == 1) {
            ...
            for ( unsigned int w=0; w<WAYS; ++w) {
              for ( unsigned int j=0; j<elementsPerCL; ++j) {
                Pair tmp = AE[w][j][gr][gc];
                bufA[w].words[j] = tmp.a;
                bufE[w].words[j] = tmp.e;
              }
            }
            */
            ...
            assert( !validBuffer);
            if ( !validPipe0 && !validPipe1 && !validPipe2 && !validPipe3) {
              localDone=1;
            }
          }
        }

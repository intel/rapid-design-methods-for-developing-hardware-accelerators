...
public:
  ...
  void inp_proc() {
    MemTypedRead<Node> mem( inpReqIn, inpRespOut);
    wait();
    while (1) {
      mem();
      wait();
    ...
    //[[[end]]] (checksum: 82e4e432aeb1eeaa02c296c1818ae63e)
  /*[[[cog
       for p in dut.inps:
  ...
  void out_proc() {
    MemTypedWrite<CycleExistInfo> mem( outReqIn, outDataIn);
    wait();
    while (1) {
      mem();
      wait();
  ...
  //[[[end]]] (checksum: 8d7427a398cc80db31b7911e940b9321)
  void init_proc() {
    start = false;

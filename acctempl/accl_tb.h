...
class acc_top_driver : public sc_module, public FpgaAppSwAlloc {
public:
  typedef CONFIG Config;
  ...
  virtual void  compute( const void * config_ptr, const unsigned int config_size) {
    tb.conf = *static_cast<const Config *> (config_ptr);
...
template <typename AU, size_t FREQ_IN_MHZ=1000, size_t LATENCY_IN_NS=128, size_t RD_PORTS=1, size_t WR_PORTS=1, size_t SPREAD_IN_NS=0>
class acc_top : public acc_top_driver<typename AU::Config, FREQ_IN_MHZ> {
public:
  typedef typename AU::Config Config;

  MockMemorySimulator<FREQ_IN_MHZ, LATENCY_IN_NS, RD_PORTS, WR_PORTS, SPREAD_IN_NS> mem;
...
template <typename AU, size_t FREQ_IN_MHZ, size_t LATENCY_IN_NS, size_t SPREAD_IN_NS>
  class acc_top<AU, FREQ_IN_MHZ, LATENCY_IN_NS, 1, 1, SPREAD_IN_NS> : public acc_top_driver<typename AU::Config, FREQ_IN_MHZ> {
public:

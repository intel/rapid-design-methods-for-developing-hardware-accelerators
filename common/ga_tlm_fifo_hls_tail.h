...
class tlm_fifo_hier_in<PAYLOAD, HLS_FLEX>: public hier_get_initiator<PAYLOAD>
{
public:
	tlm_fifo_hier_in() : hier_get_initiator<PAYLOAD>() {

	}

	tlm_fifo_hier_in(const char *name) : hier_get_initiator<PAYLOAD>(name) {
...
template<typename PAYLOAD>
class tlm_fifo_hier_out<PAYLOAD, HLS_FLEX> : public hier_put_initiator<PAYLOAD>
{
public:
	tlm_fifo_hier_out() : hier_put_initiator<PAYLOAD>() {
	}

	tlm_fifo_hier_out(const char *name) : hier_put_initiator<PAYLOAD>(name) {
	}
	template <typename CLK_T, typename RST_T>
	void clk_rst(CLK_T clk, RST_T rst) { }
...
template<typename PAYLOAD>
class tlm_fifo_in<PAYLOAD, HLS_FLEX> : public nb_get_initiator<PAYLOAD>
{
public:
  tlm_fifo_in() : nb_get_initiator<PAYLOAD>() {
  }


  tlm_fifo_in(const char *name) : nb_get_initiator<PAYLOAD>(name) {
...
template<typename PAYLOAD>
class tlm_fifo_out<PAYLOAD, HLS_FLEX> : public nb_put_initiator<PAYLOAD>
{
public:

  tlm_fifo_out() : nb_put_initiator<PAYLOAD>() {
  }

  tlm_fifo_out(const char *name) : nb_put_initiator<PAYLOAD>(name) {
  }
...
class tlm_fifo<PAYLOAD, SIZE, HLS_FLEX> : public put_get_channel<PAYLOAD>
{
public:
  tlm_fifo(sc_module_name modname) :
    put_get_channel<PAYLOAD>(modname)
  {
  }

  tlm_fifo() :
    put_get_channel<PAYLOAD>()
  {
  }
